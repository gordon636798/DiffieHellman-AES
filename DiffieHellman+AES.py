...
#AES 加密
    def AES_encrypt(self, _user, data):
        ...
        padding = 16 - len(data) % 16  #AES為block ciper，須將明文鋪滿為128 bits 的倍數
        if padding == 0 :  #缺3個bytes 就補3個 b'/x03' 
            padding = 16
        P = bytes([padding])
        data += padding*(P)
...
#AES 解密    
    def AES_decrypt(self, cip):
        ...
        plain = plain[:(len(plain)-plain[-1])] #解密時將當初的padding給去掉 
